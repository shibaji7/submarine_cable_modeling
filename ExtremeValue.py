...
def compile_cable_to_calculate_parameters(FRD_files, STJ_files, HAD_files):
    ...
    tlines.append(
        TransmissionLine(
            ...
            directed_length=dict(
                edge_locations=dict(
                    initial=dict(lat=47.85, lon=-9.05), final=dict(lat=50.79, lon=-4.55)
                )
            ),
            elec_params=dict(
                site=PROFILES.CS_E,
...
for mag in magnetometers:
    for year in years:
        dates = [dt.datetime(year, 1, 1), dt.datetime(year + 1, 1, 1)]
        expected_dp = (dates[1] - dates[0]).total_seconds() / 60.0
        dates_str = [
...
for year in years:
    ...
    if not os.path.exists(fname):
        tlines, cable = compile_cable_to_calculate_parameters(
            stj_files, stj_files, stj_files
        )
    ...
    df.to_csv(
        fname.replace(".csv", ".hr.csv"), header=True, index=False, float_format="%g"
    )
