...
class ConductivityProfile(object):
    ...
    def __init__(self):
        self.earth_model = "LITHO1.0.nc"
        
        # NOTE these are specified in terms of resistivity, in ohm-m
        self.seawater_resistivity = 0.3
        self.sediment_resistivity = 3.
        self.crust_resistivity = 3000.
        self.lithosphere_resistivity = 1000.
        self.asthenosphere_resistivity = 100.
        self.transition_zone_resistivity = 10.
        self.lower_mantle_resistivity = 1.

        # fixed layer depths (below the lithosphere), in km
        self.transition_zone_top = 410.
        self.transition_zone_bot = 660.
        self.profile_max_depth = 1000.
        
        # things that control how this code works in detail...
        # Options: "nearest" or "linear"
        self.grid_interpolation_method = "nearest"   
    ...
    def load_earth_model(self):
        ...
        """
        filename = "input/" + self.earth_model
        if not os.path.exists(filename): os.system(f"wget -O {fname} http://ds.iris.edu/files/products/emc/emc-files/LITHO1.0.nc")
        with netcdf_file(filename) as f: 
            ...
            asthenospheric_mantle_top_depth = np.copy(f.variables["asthenospheric_mantle_top_depth"][:])
            ...
            # crustal layers
            lower_crust_bottom_depth = np.copy(f.variables["lower_crust_bottom_depth"][:])
            lower_crust_top_depth = np.copy(f.variables["lower_crust_top_depth"][:])
            middle_crust_bottom_depth = np.copy(f.variables["middle_crust_bottom_depth"][:])
            middle_crust_top_depth = np.copy(f.variables["middle_crust_top_depth"][:])
            upper_crust_bottom_depth = np.copy(f.variables["upper_crust_bottom_depth"][:])
            upper_crust_top_depth = np.copy(f.variables["upper_crust_top_depth"][:])
            # sediment layers
            lower_sediments_bottom_depth = np.copy(f.variables["lower_sediments_bottom_depth"][:])
            lower_sediments_top_depth = np.copy(f.variables["lower_sediments_top_depth"][:])
            middle_sediments_bottom_depth = np.copy(f.variables["middle_sediments_bottom_depth"][:])
            middle_sediments_top_depth = np.copy(f.variables["middle_sediments_top_depth"][:])
            upper_sediments_bottom_depth = np.copy(f.variables["upper_sediments_bottom_depth"][:])
            upper_sediments_top_depth = np.copy(f.variables["upper_sediments_top_depth"][:])
            # water levels
        ...
        self.lithosphere_model = {
            ...
            "asthenospheric_mantle_top_depth": \
                    RegularGridInterpolator((latitude, longitude), asthenospheric_mantle_top_depth, 
                                            method=self.grid_interpolation_method, bounds_error=True),
            "lithosphere_bottom_depth": \
                    RegularGridInterpolator((latitude, longitude), lithosphere_bottom_depth, 
                                            method=self.grid_interpolation_method, bounds_error=True),
            "lithosphere_top_depth": \
                    RegularGridInterpolator((latitude, longitude), lithosphere_top_depth, 
                                            method=self.grid_interpolation_method, bounds_error=True),
            "lower_crust_bottom_depth": \
                    RegularGridInterpolator((latitude, longitude), lower_crust_bottom_depth, 
                                            method=self.grid_interpolation_method, bounds_error=True),
            "lower_crust_top_depth": \
                    RegularGridInterpolator((latitude, longitude), lower_crust_top_depth, 
                                            method=self.grid_interpolation_method, bounds_error=True),
            "middle_crust_bottom_depth": \
                    RegularGridInterpolator((latitude, longitude), middle_crust_bottom_depth, 
                                            method=self.grid_interpolation_method, bounds_error=True),
            "middle_crust_top_depth": \
                    RegularGridInterpolator((latitude, longitude), middle_crust_top_depth,                                  
                                            method=self.grid_interpolation_method, bounds_error=True),
            "upper_crust_bottom_depth": \
                    RegularGridInterpolator((latitude, longitude), upper_crust_bottom_depth, 
                                            method=self.grid_interpolation_method, bounds_error=True),
            "upper_crust_top_depth": \
                    RegularGridInterpolator((latitude, longitude), upper_crust_top_depth, 
                                            method=self.grid_interpolation_method, bounds_error=True),
            "lower_sediments_bottom_depth": \
                    RegularGridInterpolator((latitude, longitude), lower_sediments_bottom_depth, 
                                            method=self.grid_interpolation_method, bounds_error=True),
            "lower_sediments_top_depth": \
                    RegularGridInterpolator((latitude, longitude), lower_sediments_top_depth, 
                                            method=self.grid_interpolation_method, bounds_error=True),
            "middle_sediments_bottom_depth": \
                    RegularGridInterpolator((latitude, longitude), middle_sediments_bottom_depth, 
                                            method=self.grid_interpolation_method, bounds_error=True),
            "middle_sediments_top_depth": \
                    RegularGridInterpolator((latitude, longitude), middle_sediments_top_depth, 
                                            method=self.grid_interpolation_method, bounds_error=True),
            "upper_sediments_bottom_depth": \
                    RegularGridInterpolator((latitude, longitude), upper_sediments_bottom_depth, 
                                            method=self.grid_interpolation_method, bounds_error=True),
            "upper_sediments_top_depth": \
                    RegularGridInterpolator((latitude, longitude), upper_sediments_top_depth, 
                                            method=self.grid_interpolation_method, bounds_error=True),
            "water_bottom_depth": \
                    RegularGridInterpolator((latitude, longitude), water_bottom_depth, 
                                            method=self.grid_interpolation_method, bounds_error=True),
            "water_top_depth": \
                    RegularGridInterpolator((latitude, longitude), water_top_depth, 
                                            method=self.grid_interpolation_method, bounds_error=True),
        }
        return
    ...
    def get_interpolation_points(self, pt0, pt1):
        ...
        npts = int(round(np.sqrt((pt1[1] - pt0[1])**2. + (pt1[0] - pt0[0])**2.)))
        if npts < 1: npts = 1
        ...
        interpolation_points = np.vstack((np.array([pt0[1], pt0[0]]), 
                                          np.array(latlons), 
                                          np.array([pt1[1], pt1[0]])))

        # now swap back to [lat, lon]
        interpolation_points = np.vstack((interpolation_points[:, 1], 
                                          interpolation_points[:, 0])).T
    ...
    def get_sediment_layer(self, lithosphere_model, pts):
        ...
        sed_thk_values = np.nansum(
            np.vstack((upper_sed_thk_values, middle_sed_thk_values, lower_sed_thk_values)),
            axis=0)
    ...
    def get_crust_layer(self, lithosphere_model, pts):
        ...
        crust_thk_values = np.nansum(
            np.vstack((upper_crust_thk_values, middle_crust_thk_values, lower_crust_thk_values)),
            axis=0)
    ...
    def get_lithosphere_layer(self, lithosphere_model, pts):
        ...
        asthenosphere_top_values = lithosphere_model["asthenospheric_mantle_top_depth"](pts)
        ...
        if litho_bot != astheno_top:
            logger.warning(f"PROBLEM: lithosphere-asthenosphere dont line up: {litho_bot}, {astheno_top}")
    ...
    def _compile_profile_(self, pts):
        ...
        resistivity_profile = np.array([[water_thk, self.seawater_resistivity],
        ...
        rf = pd.DataFrame()
        rf["thickness"], rf["resistivity"] = resistivity_profile[:,0], resistivity_profile[:,1]
        return rf
    ...
    def compile_profiles(self, latlons, kind="rounded"):
        ...
        for latlon in latlons:
            if kind == "rounded": latlon = np.rint(latlon)
