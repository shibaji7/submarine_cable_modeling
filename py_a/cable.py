...
class EventAnalysis(object):
    ...
    def calclulate_total_parameters(self):
        ...
        U0x, U1x = self.nodal_analysis.get_voltage_ends_of_cable("X")
        U0y, U1y = self.nodal_analysis.get_voltage_ends_of_cable("Y")
        U0, U1 = U0x + U0y, U1x + U1y
    ...
    def synthetic_E_field_simulation(self):
        """
        ...
        Va, La = [], []
        lengths = self.E_syn["lengths"]
        if len(lengths) < len(self.tx_syn_lines): lengths += ([None]*(len(self.tx_syn_lines)-len(lengths)))
        for l, tx in enumerate(self.tx_syn_lines):
            pname = pdir + "VCable%02d.png"%l
            U0x, U1x = self.node_syn_anl.get_voltage_ends_of_cable_section(l, "X")
            U0y, U1y = self.node_syn_anl.get_voltage_ends_of_cable_section(l, "Y")
            U0, U1 = U0x + U0y, U1x + U1y
            V, Lx = tx.calculate_potential_along_cable_section(U0, U1, plot=True, 
                                                               pname=pname, L=lengths[l])
            Va.extend(V.tolist())
            if l==0: La = Lx.tolist()
            else: La.extend((Lx+La[-1]).tolist())
        plotlib.cable_potential(Va, La, pdir + "VCable.png")
        return
