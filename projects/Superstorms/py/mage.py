...
def fetch_MAGE_simulations(stations=["frd", "stj", "had"]):
    ...
    for stn in stations:
        ...
        d["Date"] = [
            dt.datetime(2024, 5, 9, 20) + dt.timedelta(minutes=i) for i in range(len(d))
        ]
        d = d[d.Date < dt.datetime(2024, 5, 12)]
        d.to_csv(
            f"{folder}{stn.lower()}_May2024_MAGE.csv",
            index=False,
            header=True,
            float_format="%g",
        )
...
def stn_validation_plots(
    stn, dates=[dt.datetime(2024, 5, 10, 12), dt.datetime(2024, 5, 12)]
):
    global folder, base_folder

    from supermag import SuperMAGGetData, sm_grabme

    (_, data) = SuperMAGGetData(
        "shibaji7", dates[0], 3600 * 36, "all,delta=start,baseline=yearly", stn.upper()
    )
    if len(data) > 0:
        for comp in ["N", "E", "Z"]:
            for cord in ["nez", "geo"]:
        ...
        data.tval = data.tval.apply(lambda x: dt.datetime.utcfromtimestamp(x))
    ...
    # Add Intermagnet
    ts.add_curve(
        o.Date, o.Y, ax=ax, color="b", ylabel=r"$N_{nez}(Y)$ [nT]", ylim=[-1000, 1000]
    )
    ...
    ts.add_curve(
        o.Date, o.X, ax=ax, color="b", ylabel=r"$N_{nez}(X)$ [nT]", ylim=[-1000, 1000]
    )
...
def run_May2024_storm():
    ...
    data.to_csv(
        "simulation/May2024/SCUBAS-Simulation-MAGE.csv", float_format="%g", index=False
    )
    return


def SuperMAG_compare_plots():
    smag_simulations = pd.read_csv(
        "simulation/May2024/SCUBAS-Simulation-SuperMAG-Fitted.csv", parse_dates=["Time"]
    ).set_index("Time")
    print(smag_simulations.head())
    scuba_simulations = pd.read_csv(
        "simulation/May2024/SCUBAS-Simulation-3-Stations.csv", parse_dates=["Time"]
    ).set_index("Time")
    mage_simulations = pd.read_csv(
        "simulation/May2024/SCUBAS-Simulation-MAGE.csv", parse_dates=["Time"]
    ).set_index("Time")
    ...
    ts.add_voltage(
        scuba_simulations, ax=ax, color="r", xlabel="Hours Since 10 May 2024, 12 UT"
    )
