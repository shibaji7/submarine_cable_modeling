...
def fetch_data_by_station(stn="frd"):
    ...
    dates = [
        dt.datetime(2024, 5, 10) + dt.timedelta(minutes=t) for t in range(1440 * 2)
    ]
...
def interpolate_smag():
    ...
    dates = [
        dt.datetime(2024, 5, 10) + dt.timedelta(minutes=t) for t in range(2 * 1440)
    ]
    for i, seg in enumerate(cable.cable_seg):
        ...
        if not os.path.exists(fname):
            ...
            frame.rename(
                columns=dict(
                    dates="Date", dbn_nez="X", dbe_nez="Y", dbz_nez="Z", db_nez="F"
                ),
                inplace=True,
            )
            frame.to_csv(fname, float_format="%g", index=False, header=True)
...
def run_May2024_storm():
    ...
    data.to_csv(
        "simulation/May2024/SCUBAS-Simulation-SuperMAG-Fitted.csv",
        float_format="%g",
        index=False,
    )
    return
