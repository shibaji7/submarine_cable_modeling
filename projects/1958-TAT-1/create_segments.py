...
ax.plot(
    data.Time,
    data.X - np.nanmedian(data.X.iloc[0:60]),
    color="b",
    lw=0.6,
    ls="-",
    label=r"$B_x$",
...
ax.plot(
    data.Time,
    data.Y - np.nanmedian(data.Y.iloc[0:60]),
    color="r",
    lw=0.6,
    ls="-",
    label=r"$B_y$",
...
ax.plot(
    data.Time,
    data.Z - np.nanmedian(data.Z.iloc[0:60]),
    color="g",
    lw=0.6,
    ls="-",
    label=r"$B_z$",
...
data[["Date", "X", "Y", "Z", "F"]].to_csv(
    "dataset/compiled.csv", float_format="%g", index=False
)
...
ax = ts.add_voltage(
    model0.cable.tot_params,
    lw=0.7,
    color="k",
    ylim=[-3000, 3000],
    xlabel="UT since 11 UT on 10 Feb, 1958",
)
ax.plot(
    model0.cable.tot_params.index,
    model0.cable.tot_params["V(v)"],
    color="r",
    ls="-",
    lw=0.7,
    alpha=0.7,
...
ax = ts.add_voltage(
    ...
    color="k",
    ylim=[-3000, 3000],
    xlabel="Staring at 00 UT, 11 Feb, 1958",
)
ax.plot(
    model1.cable.tot_params.index,
    model1.cable.tot_params["Vt(v)"],
    color="r",
    ls="-",
    lw=0.7,
    alpha=0.7,
...
ax.plot(
    model2.cable.tot_params.index,
    model2.cable.tot_params["Vt(v)"],
    color="b",
    ls="-",
    lw=0.7,
    alpha=0.7,
    zorder=5,
    label="AT/Land,50-m Water",
)
