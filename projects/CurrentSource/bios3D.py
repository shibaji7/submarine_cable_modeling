...
def biot_savart_3d_fem(nodes, elements, J, evaluation_points, quadrature_points=3):
    ...
    gauss_points, gauss_weights = np.polynomial.legendre.leggauss(quadrature_points)  # Gauss-Legendre quadrature
    for element in elements:
        ...
        for i, point in enumerate(evaluation_points):
             ...
             J_value = J(center_point[0], center_point[1], center_point[2]) #Current Density
             dB = (mu_0 / (4 * np.pi)) * (np.cross(J_value, r) / (np.linalg.norm(r)**3))
             B[i]+=dB
...
height = 1 # Length
nodes, elements = create_cylinder_mesh(radius, height, n_radial=4, n_circumferential=8, n_axial=10)
...
#Define the current
evaluation_points = np.array([[0,0,0],[.09,.09,.09],[.001,.01,.2],[1,1,1]])
