...
def plot_real_imag_plots(c, d):
    ...
    fig, axes = plt.subplots(
        nrows=2, ncols=2, dpi=240, figsize=(10,6)
    )
    axes = axes.ravel()
    ax = axes[0]
    ax.plot(d/1e3, 1e9*np.real(c.B_fields[0]["total"]), ls="-", color="k", lw=1.2, label="Total")
    ax.plot(d/1e3, 1e9*np.real(c.B_fields[0]["external"]), ls="--", color="r", lw=0.8, label="External")
    ax.plot(d/1e3, 1e9*np.real(c.B_fields[0]["internal"]), ls="--", color="b", lw=0.8, label="Internal")
    ax.set_xlim(d[0]/1e3, d[-1]/1e3)
    ax.legend(loc=1)
    
    ax = axes[1]
    ax.plot(d/1e3, 1e9*np.real(c.B_fields[1]["total"]), ls="-", color="k", lw=1.2)
    ax.plot(d/1e3, 1e9*np.real(c.B_fields[1]["external"]), ls="--", color="r", lw=0.8)
    ax.plot(d/1e3, 1e9*np.real(c.B_fields[1]["internal"]), ls="--", color="b", lw=0.8)
    ax.set_xlim(d[0]/1e3, d[-1]/1e3)

    ax = axes[2]
    ax.plot(d/1e3, 1e9*np.imag(c.B_fields[0]["total"]), ls="-", color="k", lw=1.2)
    ax.plot(d/1e3, 1e9*np.imag(c.B_fields[0]["external"]), ls="--", color="r", lw=0.8)
    ax.plot(d/1e3, 1e9*np.imag(c.B_fields[0]["internal"]), ls="--", color="b", lw=0.8)
    ax.set_xlim(d[0]/1e3, d[-1]/1e3)
    
    ax = axes[3]
    ax.plot(d/1e3, 1e9*np.imag(c.B_fields[1]["total"]), ls="-", color="k", lw=1.2)
    ax.plot(d/1e3, 1e9*np.imag(c.B_fields[1]["external"]), ls="--", color="r", lw=0.8)
    ax.plot(d/1e3, 1e9*np.imag(c.B_fields[1]["internal"]), ls="--", color="b", lw=0.8)
    ax.set_xlim(d[0]/1e3, d[-1]/1e3)
    ...
    fig, axes = plt.subplots(
        nrows=2, ncols=1, dpi=240, figsize=(5,6)
    )
    axes = axes.ravel()
