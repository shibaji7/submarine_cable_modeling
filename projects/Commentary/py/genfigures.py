...
def figure2():
    ...
    ax.axvline(1e-2, ls="--", lw=0.8, color="m")
    ax.set_xticklabels([])
    ax.text(
        0.9,
        1.05,
    ...
    ax.semilogx(
        tfs[0].freq, np.angle(tfs[0].E2B, deg=True), "r", lw=1.0, ls="-", label="Non Uniform"
    )
    ax.semilogx(
        tfs[2].freq,
...
def figure3():
    ...
    ax.set_xlim(flim)
    ax.set_xticks([1e-10, 1e-5, 1e-1, 1e0])
    ax.set_xticklabels([])
    ax.set_ylim(1e-3, 1e3)
    ax.set_yticks([1e-3, 1e0, 1e3])
    ax.axvline(1e-5, ls="--", lw=0.8, color="m")
    ax.axvline(1e-1, ls="--", lw=0.8, color="m")
    ax.text(
        0.9,
        1.05,
    ...
    ax = fig.add_subplot(212)
    ax.semilogx(
        tfs[1].freq, np.angle(tfs[1].E2B, deg=True), "r", lw=1.0, ls="-", label="Non Uniform"
    )
    ax.semilogx(
        tfs[2].freq,
        np.angle(tfs[2].E2B, deg=True),
