...
def compute_segmented_corr(ds, dth, s0, s1, sub1, sub2):
    ...
    for tc in theta_bins:
        tcs.append(tc - 270)
        tl, th = tc - dth, tc + dth
        o = ds[(ds.theta_Eh >= tl) & (ds.theta_Eh < th)]
...
def pipeline():
    ...
    thetaA, CorA_B, CorA_dB = compute_segmented_corr(
        dsA, dth=10, s0=0.035, s1=0.15, sub1=3e-2, sub2=0
    )
    dsB = compute(1)
    thetaB, CorB_B, CorB_dB = compute_segmented_corr(
        dsB, dth=10, s0=1e-4, s1=0.1, sub1=0.4, sub2=5e-2
    )
...
def compute_segmented_stn_corr(stn, ds, dth, s0, s1, sub1, sub2):
    ...
    ds["GIC"] = 1e-3 * (stnab["a"] * np.array(ds.Ex) + (stnab["b"] * np.array(ds.Ey)))
    ...
    Cor_B, Cor_dB = (set_last_mid_point(Cor_B), set_last_mid_point(Cor_dB))
    # theta, Cor_B = smooth_filling(tcs, Cor_B, s0)
    # _, Cor_dB = smooth_filling(tcs, Cor_dB, s1)
    # Cor_B, Cor_dB = (
    #     set_last_mid_point(Cor_B),
...
def powernet(stn=6):
    ...
    thetaA, CorA_B, CorA_dB = compute_segmented_stn_corr(
        f"stn{stn}", dsA, dth=5, s0=0.02, s1=0.08, sub1=3e-2, sub2=0
    )

    thetaB, CorB_B, CorB_dB = compute_segmented_stn_corr(
        f"stn{stn}", dsB, dth=5, s0=0.02, s1=0.1, sub1=0.1, sub2=5e-2
    )
